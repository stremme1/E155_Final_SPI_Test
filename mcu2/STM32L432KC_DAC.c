...
void DAC_PlaySineWave(float frequency, uint32_t duration_ms, uint32_t sample_rate) {
    ...
    // CALIBRATED: Based on frequency measurements (matches working mcu/ version)
    ...
    #define CPU_FREQ_MHZ 18  // Calibrated CPU frequency in MHz (matches working mcu/ version)
...
void DAC_PlayWAV(const int16_t* sample_data, uint32_t sample_length, uint32_t sample_rate) {
    ...
    // NOTE: Even though system clock is 80MHz, the calibrated effective frequency is 15MHz
    // This accounts for wait states, cache, and other factors affecting actual execution speed
    #undef CPU_FREQ_MHZ  // Undefine previous definition
    #define CPU_FREQ_MHZ 15  // Calibrated CPU frequency in MHz (matches working mcu/ version)
    uint32_t us_per_sample = 1000000UL / sample_rate;
    uint32_t total_cycles_needed = us_per_sample * CPU_FREQ_MHZ;
