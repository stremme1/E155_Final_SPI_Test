...
- `arduino_spi_slave.sv` — Receives 16-byte packets from Arduino via SPI. Buffers the packet and performs real-time quaternion-to-Euler angle conversion using dedicated DSP blocks (SB_MAC16 primitives on iCE40UP5k). The conversion uses 9 DSP block multiplications for quaternion math (w*x, y*z, w*y, z*x, w*z, x*y, x*x, y*y, z*z) plus 3 additional DSP blocks for scaling to degrees, ensuring efficient hardware-accelerated computation. The converted Euler angles (Roll, Pitch, Yaw) along with gyroscope data are packaged into the output packet format (Header, Roll, Pitch, Yaw, Gyro X/Y/Z, Flags) and synchronized to the FPGA clock domain.
...
*Figure: Final Dual-Stick Embedded System Prototype*

::: {.section-title-container}
## FPGA Design Details {.section-title}
:::

Initially, we attempted to configure the BNO085 sensors directly from the FPGA using a 11-state finite state machine (FSM) with encoding logic implemented using DSP blocks. This approach aimed to handle the Sensor Hub Transport Protocol (SHTP) communication directly on the FPGA, including sensor initialization, command encoding, and data packet parsing. While this implementation successfully extracted data from the sensors (as shown in the figure below), the approach proved unreliable in practice.
...
2. **FPGA**: Receives pre-processed sensor data from the Arduino via SPI and performs real-time quaternion-to-Euler angle conversion using dedicated DSP blocks. The FPGA uses 12 DSP block multiplications (9 for quaternion cross-products and squares, 3 for degree scaling) to efficiently convert the incoming quaternion orientation data to Euler angles. The converted data is then packaged into a custom transmission format optimized for the MCU interface. The FPGA's role shifted from sensor management to efficient data processing using hardware-accelerated DSP blocks and data routing.
...
`drum_trigger_top.sv` instantiates two submodules that handle the SPI interfaces on each side: `arduino_spi_slave.sv`, which implements the SPI Mode 0 slave interface to the Arduino by receiving 16-byte raw sensor packets containing quaternion data, shifting the data in during each SCK rising edge, synchronizing it into the FPGA clock domain, and performing quaternion-to-Euler conversion using dedicated DSP blocks. The module uses synthesis attributes (`(* use_dsp = "yes" *)`) to explicitly force the use of DSP blocks (SB_MAC16 primitives) for all multiplication operations, ensuring optimal hardware resource utilization. The converted Euler angles are packaged with gyroscope data and exposed as a stable packet buffer for downstream processing. The second module, `spi_slave_mcu.sv`, acts as a read-only SPI Mode 0 slave to the MCU, outputting those 16-byte packets on MISO as the MCU generates dummy clock cycles, enabling direct transfer of the processed sensor data to the MCU for parsing and drum-trigger computation.

### DSP Block Implementation for Quaternion-to-Euler Conversion

The `arduino_spi_slave.sv` module implements hardware-accelerated quaternion-to-Euler angle conversion using the iCE40UP5k's dedicated DSP blocks. The conversion requires computing the following Euler angle formulas:

- **Roll**: `atan2(2*(w*x + y*z), 1 - 2*(x*x + y*y))`
- **Pitch**: `asin(2*(w*y - z*x))`
- **Yaw**: `atan2(2*(w*z + x*y), 1 - 2*(y*y + z*z))`

To compute these formulas efficiently, the module performs 9 quaternion multiplications using DSP blocks:
- Cross-products: `w*x`, `y*z`, `w*y`, `z*x`, `w*z`, `x*y`
- Squares: `x*x`, `y*y`, `z*z`

Each multiplication is explicitly marked with the synthesis attribute `(* use_dsp = "yes" *)`, which forces the synthesis tool to map these operations to SB_MAC16 DSP primitives on the iCE40UP5k. The iCE40UP5k contains 8 DSP blocks, each supporting 18×18-bit signed multiplication, making it well-suited for this application.

Additionally, 3 more DSP blocks are used for scaling the intermediate Euler angle values to degrees (multiplying by 18000 and dividing by 16384 to convert from Q14 fixed-point format to degrees scaled by 100).

This DSP-based implementation provides several advantages:
- **Performance**: DSP blocks execute multiplications in a single clock cycle, significantly faster than LUT-based implementations
- **Resource Efficiency**: Frees up general-purpose logic (LUTs) for other functions
- **Power Efficiency**: Dedicated DSP blocks consume less power than equivalent LUT implementations
- **Deterministic Timing**: Hardware multipliers provide consistent, predictable latency
...
![Full Dual-Stick Embedded System Prototype](Images/image13.png){width="60%" style="max-width: 600px; margin: 2rem auto; display: block;"}

*Figure: Full Dual-Stick Embedded System Prototype*

![User Testing of the Dual-Stick Invisible Drum Set Prototype](Images/image22.jpg){width="60%" style="max-width: 600px; margin: 2rem auto; display: block;"}

*Figure: User Testing of the Dual-Stick Invisible Drum Set Prototype*

![Final Prototype Assembly with Integrated Speaker Output](Images/image16.png){width="60%" style="max-width: 600px; margin: 2rem auto; display: block;"}
...
![Arduino Serial Monitor Sensor Data](Images/image21.png){width="60%" style="max-width: 600px; margin: 2rem auto; display: block;"}
