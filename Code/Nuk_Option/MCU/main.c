...
// Global variable to store Euler angles from Arduino
static euler_t euler_from_arduino = {0.0f, 0.0f, 0.0f};
static uint8_t euler_valid = 0;
...
void process_sensor_data(sensor_data_t *data) {
    ...
    // Only process if we have valid Euler data from Arduino
    if (!euler_valid && !data->quat2_valid) {
        debug_print("[SENSOR] No valid sensor data, skipping processing\r\n");
        return;
    }
    
    // Use Euler angles directly from Arduino (Sensor 1)
    if (euler_valid) {
        euler1 = euler_from_arduino;  // Use Euler angles directly (already converted on Arduino)
        debug_printf("[SENSOR] Euler1 (from Arduino): roll=%.2f pitch=%.2f yaw=%.2f\r\n", 
                     euler1.roll, euler1.pitch, euler1.yaw);
    } else {
        // Fallback: convert quaternion to Euler if available (shouldn't happen with Arduino)
        if (data->quat1_valid) {
            quaternion_to_euler(data->quat1_w, data->quat1_x, data->quat1_y, data->quat1_z, &euler1);
            debug_printf("[SENSOR] Euler1 (from quaternion): roll=%.2f pitch=%.2f yaw=%.2f\r\n", 
                         euler1.roll, euler1.pitch, euler1.yaw);
        } else {
