...
void loop() {
  ...
  // Send packet via SPI to MCU using default SPI pins
  // Note: This is a simplified example - in production, use proper packet format
  // with header 0xAA, Euler angles, gyro data, and flags
  uint8_t packet[16];
  packet[0] = 0xAA;  // Header
  // Pack yaw as int16_t (scaled by 100 for 0.01 degree resolution)
  int16_t yaw_scaled = (int16_t)(yaw * 100.0f);
  packet[1] = (yaw_scaled >> 8) & 0xFF;  // Yaw MSB
  packet[2] = yaw_scaled & 0xFF;          // Yaw LSB
  // Fill rest with zeros (simplified example)
  for (int i = 3; i < 16; i++) {
    packet[i] = 0x00;
  }
