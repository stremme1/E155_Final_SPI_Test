...
// - Detects drum triggers using LEFT HAND zone detection
...
void read_sensor_data(sensor_data_t *data) {
    ...
    // Parse packet into structure (treating as sensor 1, which we'll use for left hand)
    // Note: The packet contains one sensor's data, we'll use sensor 1 fields
    ...
    parseSensorDataPacket(packet,
                          // Sensor 1 (we'll use this for left hand data)
    ...
    // Copy sensor 1 data to our structure (treating it as left hand)
    data->quat_w = quat1_w;
    data->quat_x = quat1_x;
    data->quat_y = quat1_y;
    data->quat_z = quat1_z;
...
// Process sensor data and detect drum triggers (LEFT HAND ONLY)
void process_sensor_data(sensor_data_t *data) {
    ...
    debug_printf("[SENSOR] Left Hand Euler: roll=%.2f pitch=%.2f yaw=%.2f\r\n", euler.roll, euler.pitch, euler.yaw);
    
    // Detect drum trigger using LEFT HAND logic
    drum_code = detect_drum_trigger_left(
        &euler,
        data->gyro_y,
        data->gyro_z,
        &drum_state,
        ...
        if (debounce_counter > 1000) {  // Debounce threshold
            ...
            if (current_state == 0) {  // Button pressed (LOW when pressed)
                ...
                if (data.quat_valid) {
                    quaternion_to_euler(data.quat_w, data.quat_x, data.quat_y, data.quat_z, &euler);
                    // Update offset
                    update_yaw_offset_left(&euler, &yaw_offset);
                    debug_printf("[BUTTON] Left hand yaw offset updated: offset=%.2f\r\n", yaw_offset);
                }
            }
        }
    } else {
...
int main(void) {
    ...
    debug_print("\r\n\r\n=== MCU Drum Trigger System - LEFT HAND ===\r\n");
    ...
    debug_print("[MAIN] Entering main loop - reading LEFT HAND sensor data from FPGA\r\n");
