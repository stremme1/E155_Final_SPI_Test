...
void test_spi_parser(void) {
    ...
    // Verify Sensor 2 is set to 0/invalid (not in 16-byte packet)
    assert(quat2_w == 0 && quat2_x == 0 && quat2_y == 0 && quat2_z == 0);
    assert(gyro2_x == 0 && gyro2_y == 0 && gyro2_z == 0);
    assert(quat2_valid == 0 && gyro2_valid == 0);
    printf("  [PASS] Sensor 2 correctly set to 0/invalid (single-sensor packet)\n");
